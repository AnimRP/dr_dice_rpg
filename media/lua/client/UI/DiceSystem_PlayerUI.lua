...
function DiceMenu:update()
    ...
    if not isInit or isAdmin then
        ...
        local pointsAllocatedString = getText("IGUI_SkillPointsAllocated") .. string.format(" %d/", allocatedPoints) .. tostring(total_max_points)
        ...
        -- Save button
        self.btnConfirm:setEnable(allocatedPoints == total_max_points)
    else
    ...
    for i = 1, #PLAYER_DICE_VALUES.SKILLS do
        ...
        if bonusSkillPoints ~= 0 then
            local operand = "+"
            local color = "<RGB:0.94,0.82,0.09>"
            if bonusSkillPoints < 0 then
                operand = "-"
                color = "<RGB:0.94,0.19,0.19>"
            end
            
            skillPointsString = skillPointsString ..
            string.format(" %s <SPACE> %s <SPACE> %d", color, operand, math.abs(bonusSkillPoints))
        end
        ...
        if not isInit or isAdmin then
            self["btnMinus" .. skill]:setEnable(skillPoints ~= PLAYER_DICE_VALUES.MIN_PER_SKILL_ALLOCATED_POINTS)
