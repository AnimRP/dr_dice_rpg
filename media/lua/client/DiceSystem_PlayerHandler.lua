...
function PlayerHandler:incrementSkillPoint(skill)
    local result = false
    local total_max_points = PLAYER_DICE_VALUES["MAX_ALLOCATED_POINTS"]
    local attribute_max_points = PLAYER_DICE_VALUES["MAX_PER_SKILL_ALLOCATED_POINTS"]

    if self.diceData.allocatedPoints < total_max_points and self.diceData.skills[skill] < attribute_max_points then
...
function PlayerHandler:handleSkillPoint(skill, operation)
    ...
    self:updateAttributeBonus(skill, self.diceData.skills[skill])
    ...
    if skill == 'Endurance' or skill == 'Dexterity' then
        local end_bonus = self:getBonusSkillPoints("Endurance")
        local dex_bonus = self:getBonusSkillPoints("Dexterity")
        
        local movement_bonus = end_bonus + dex_bonus
        self:applyMovementBonus(movement_bonus)
    end

    if skill == "Endurance" or skill == "Dexterity" or skill == "Strength" then
        local end_bonus = self:getBonusSkillPoints("Endurance")
        local dex_bonus = self:getBonusSkillPoints("Dexterity")
        local str_bonus = self:getBonusSkillPoints("Strength")

        local evasion_bonus = end_bonus + dex_bonus + str_bonus
        self:applyEvasionBonus(evasion_bonus)
    end
    return result
end
...
function PlayerHandler:updateAttributeBonus(attribute, points)
    local bonus = DiceSystem_Common.GetAttributeBonus(points)
    self.diceData.skillsBonus[attribute] = bonus
end
...
function PlayerHandler:getMaxHealth()
    if DICE_CLIENT_MOD_DATA and self.username and DICE_CLIENT_MOD_DATA[self.username] then
        max_health = DICE_CLIENT_MOD_DATA[self.username].maxHealth
...
function PlayerHandler:applyMovementBonus(BonusPoints)
    local starting_movement = PLAYER_DICE_VALUES["DEFAULT_MOVEMENT"]
    DICE_CLIENT_MOD_DATA[self.username].movementBonus = starting_movement + BonusPoints
end

---@param BonusPoints number
function PlayerHandler:applyEvasionBonus(BonusPoints)
    DICE_CLIENT_MOD_DATA[self.username].maxHealth = BonusPoints + 2
...
function PlayerHandler:getArmorClass()
    if self:checkDiceDataValidity() then
        local dexBonus = DICE_CLIENT_MOD_DATA[self.username].skillsBonus["Dexterity"]
        local endBonus = DICE_CLIENT_MOD_DATA[self.username].skillsBonus["Endurance"]

        local armorClass = dexBonus + endBonus
...
function PlayerHandler:initModData(force)
    ...
    if (DICE_CLIENT_MOD_DATA ~= nil and DICE_CLIENT_MOD_DATA[self.username] == nil) or force then
        ...
        tempTable = {
            ...
            movementBonus = PLAYER_DICE_VALUES.DEFAULT_MOVEMENT,
